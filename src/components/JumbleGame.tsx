...
export const JumbleGame: React.FC = () => {
  ...
  const [lastMove,setLastMove] = useState<{i:number,j:number}>({i:-2,j:-2});
  const [firstMove,setFirstMove]=useState(true);
  const [myString,setMyString]=useState('');
  const possible =(i:number,j:number)=>{
    if((Math.abs(i-lastMove.i)<=1 && Math.abs(j-lastMove.j)<=1) && !((Math.abs(i-lastMove.i)===1 && Math.abs(j-lastMove.j)===1))){
      return true;
    }
    return false;
  }
  ...
  return (
    ...
    >
      ...
      <Pressable onTouchStart={reset} style={{width:60,height:60,
    borderRadius:30,
    ...
    alignItems:'center'}}>
        <Text style={{fontWeight:'bold', fontSize:25, textAlign:'center'}}>X</Text>
      </Pressable>
          </View>
    </View>
  );
